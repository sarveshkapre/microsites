...
- [x] (P1) Expand a11y bar checks beyond gallery to one demo route (`premium-product`) for skip-link/focus-visible parity.
...
- [x] (P2) Add a shared “live deployment URL checker” for gallery metadata to catch stale links automatically.
  - Score: impact medium | effort medium | strategic fit medium | differentiation low | risk low | confidence medium
- [x] (P2) Add chart annotations/callouts to `dataviz-scrolly` with perf-mode fallbacks.
  - Score: impact medium | effort medium | strategic fit high | differentiation medium | risk low | confidence medium
- [x] (P2) Add a small smoke target for `editorial-scrolly` to improve Next runtime coverage.
  - Score: impact medium | effort low | strategic fit high | differentiation low | risk low | confidence high
- [x] (P2) Consolidate repeated demo header controls into a shared component in `packages/controls`.
  - Score: impact medium | effort medium | strategic fit medium | differentiation low | risk medium | confidence medium
- [x] (P3) Add a “motion contract” static check ensuring each app declares reduced-motion + perf-mode controls in README.
  - Score: impact medium | effort medium | strategic fit medium | differentiation low | risk low | confidence medium
...
- [x] (P3) Add optional idle-prefetch for deferred WebGL stage to reduce first interaction delay without inflating initial payload.
...
- [ ] (P3) Add reduced-motion visual snapshots (CI) for one Next demo + one Vite demo.
  - Score: impact medium | effort medium | strategic fit medium | differentiation low | risk medium | confidence medium
- [x] (P3) Add a docs table summarizing per-app bundle budgets and rationale.
  - Score: impact low | effort low | strategic fit medium | differentiation low | risk low | confidence high

## Implemented
- [x] (2026-02-17) Consolidated repeated demo header controls via shared `DemoControlBar` in `@microsites/controls`.
  Evidence: `packages/controls/index.js`, `packages/controls/index.d.ts`, six demo components; local: `npm run lint -w premium-product && npm run lint -w editorial-scrolly && npm run lint -w webgl-dom-sync && npm run lint -w neon-cinematic && npm run lint -w playful-micro && npm run lint -w dataviz-scrolly` (PASS).
- [x] (2026-02-17) Expanded a11y bar checks to `premium-product` (skip-link + focus-visible parity with gallery).
  Evidence: `apps/premium-product/src/app/layout.tsx`, `apps/premium-product/src/app/globals.css`, `scripts/check-a11y-bar.mjs`; local: `npm run build:pages && npm run check:a11y` (PASS).
- [x] (2026-02-17) Expanded smoke coverage with `editorial-scrolly` in `smoke:ci`.
  Evidence: `package.json` (`smoke:ci`); local: `npm run smoke:ci` (PASS).
- [x] (2026-02-17) Added chart annotations/callouts to `dataviz-scrolly` with perf-mode marker fallback.
  Evidence: `apps/dataviz-scrolly/src/components/DataVizScrollyDemo.tsx`, `apps/dataviz-scrolly/README.md`; local: `npm run build -w dataviz-scrolly` (PASS).
- [x] (2026-02-17) Added idle-prefetch for deferred `webgl-dom-sync` stage module.
  Evidence: `apps/webgl-dom-sync/src/app/_components/WebglDomSyncDemo.tsx`, `apps/webgl-dom-sync/README.md`; local: `npm run build -w webgl-dom-sync` (PASS).
- [x] (2026-02-17) Added live deploy URL checker + motion contract checker to root verification.
  Evidence: `scripts/check-deploy-urls.mjs`, `scripts/check-motion-contract.mjs`, `package.json`; local: `npm run check:deploy-urls` + `npm run check:motion-contract` (PASS).
- [x] (2026-02-17) Added per-app bundle budget rationale table in docs.
  Evidence: `docs/PERF.md`; local: `npm run check:bundles` (PASS).
...
- `npm run verify` runs `lint:all`, then `build:pages`, then `check:bundles`, then `check:a11y`, then `check:motion-contract`, then `check:deploy-urls`.
